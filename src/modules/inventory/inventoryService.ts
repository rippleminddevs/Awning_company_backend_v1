...
import { AnalyticsResponse, GetInventoryParams, Inventory, InventoryResponse } from './inventoryInterface'
...
export class InventoryService extends BaseService<Inventory> {
  ...
  private getPopulatedInventory = async(id: string): Promise<InventoryResponse> => {
    ...
    const result : any = await inventoryModel.findById(id)
    .populate({
      path: 'awningType',
      select: 'name',
      model: 'Category'
  ...
  // Get Inventory
  public getInventories = async(params: GetInventoryParams): Promise<InventoryResponse[] | PaginatedResponse<InventoryResponse>> => {
  ...
  public getInventoryAnalytics = async (): Promise<AnalyticsResponse> => {
    ...
    const [current, last] = await Promise.all([
      ...
      // Current month data
      this.model.getMongooseModel()?.aggregate([
        {
          $group: {
            _id: null,
            totalItems: { $sum: 1 },
