...
export class NotificationService extends BaseService<Notification> {
  ...
  private async getPopulatedNotification(
    id: string
  ): Promise<(Notification & NotificationMessage) | null> {
    ...
    if (notification.createdBy?.profilePicture?.url) {
      notification.createdBy.profilePicture = notification.createdBy.profilePicture.url
    }
  ...
  public createNotification = async (notificationParams: CreateNotificationParam) => {
    ...
    const populatedNotification = await this.getPopulatedNotification(
      notificationResult._id.toString()
    )
    ...
    switch (type) {
      case 'Appointment':
        ...
        if (refData?.time && refData?.firstName && refData?.lastName) {
          const timeStr = new Date(refData.time).toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
          })
  ...
  public markAsRead = async (params: MarkAsReadParams): Promise<Notification | null> => {
    ...
    const notification = await this.model.getMongooseModel().findOne({
      _id: notificationId,
      $or: [{ targets: userId }, { createdBy: userId }],
    })
