...
export class SaleService extends BaseService<Sale> {
  ...
  private async getCounts(startDate: Date, endDate: Date) {
    ...
    const soldItemsCount = soldAppointments.filter(appt => quoteMap.has(appt._id.toString())).length
  ...
  public getSalesRepresentatives = async (
  ...
  // Get current orders
  public getCurrentOrders = async (
    params: GetCurrentOrdersParams
  ): Promise<CurrentOrder[] | PaginatedResponse<CurrentOrder>> => {
    ...
    const pipeline: any[] = [
      ...
      },
      {
        $addFields: {
          invoiceUrl: {
            $ifNull: [{ $arrayElemAt: ['$invoiceUpload.url', 0] }, null],
          },
        },
      },
      {
  ...
  private async getSalesReportTableData(
    salePersonId: string,
    page: number = 1,
    perPage: number = 10
  ) {
      ...
      this.appointmentModel.getMongooseModel()?.aggregate([
        ...
        { $unwind: { path: '$quote', preserveNullAndEmptyArrays: true } },
        {
          $group: {
                    ...
                    $not: [
                      {
                        $in: [
                          '$status',
                          ['Scheduled', 'Quoted', 'Sold', 'Confirmed', 'Followed Up'],
                        ],
                      },
                    ],
                  },
            ...
            orders: {
              $sum: {
                $cond: [{ $in: ['$status', ['Sold', 'Confirmed']] }, 1, 0],
              },
            },
            quotedAmount: {
              $sum: {
  ...
  public downloadSalesReport = async (salePersonId: string, params: any): Promise<any> => {
    ...
    const uploadRecord = await this.saveReportFile(
      buffer,
      fileName,
  ...
  public getDashboardAnalytics = async (
    ...
    const [totalCustomers, newLeads, totalSalesResult, salesAnalytics, orderStats] =
      await Promise.all([
        // Total Customers
        this.appointmentModel.count({
          createdAt: { $gte: countsStartDate, $lte: now },
